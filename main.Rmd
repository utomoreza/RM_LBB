...
CheckHomos <- function(model){
     ...
     print(BP)
     if (BP$p.value <= 0.05) {
          print("H0 rejected: Error variance spreads INCONSTANTLY/generating patterns (Heteroskedasticity)")
     } else {
          print("H0 failed to reject: Error variance spreads CONSTANTLY (Homoskedasticity)")
     }
}
```
...
## Exploratory Data Analysis {.tabset .tabset-fade .tabset-pills} {#eda}
...
Those perhaps indicate sufficiently high multicollinearity. We will highlight this issue and discuss it later in the [model for red wine](#redasum).
...
## Modelling {.tabset .tabset-fade .tabset-pills} {#model}
...
#### Check the assumptions {#redasum}

Since the linearity assumption has been discussed earlier in [this section](#Using-Scatter-Plots,-Correlation,-and-p-value), here, we're going to use three assumptions, i.e. normality, homoskedasticity, and multicollinearity.
...
In homoskedasticity aspect, we'd like to have residuals spreading constantly randomly, without generating any pattern. We have two approaches to examine this aspect, i.e. plotting the residuals vs the predicted values and performing the Breusch-Pagan test. As the function `CheckHomos()` to carry out this task has been declared already in [Preparation](#prep), we just need to use it.

```{r}
CheckHomos(model = model_red1)
```

As read above, the p-value is so small that null hypothesis is rejected. Moreover, the figure above also points out line-like patterns. This indeed states that the residuals generate patterns, meaning that heteroskedasticity exists. We don't want this.
...
By looking at the scatter plots, their relationships don't indicate a strong one. We also can check the relationships by reading the VIF scores. The only largest value is `fixed.acidity`, i.e. ± 6.9. Fortunately, such score is still lower than 10. Therefore, in case of multicollinearity, our
...
## Model Improvements {.tabset .tabset-fade .tabset-pills} {#modelimprov}

### Improving Red Wine Model
...
### Improving White Wine Model

## Results and Discussions {.tabset .tabset-fade .tabset-pills} {#resdis}
