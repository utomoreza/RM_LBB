...
The performances of both models above show ------. Now, let's carry out three approaches of step-wise algorithm, i.e. backward, forward, and both.
...
```{r}
indicator(model = model.back_red, y_pred = model.back_red$fitted.values, y_true = X.train_red$quality)
```

```{r}
indicator(model = model.forw_red, y_pred = model.forw_red$fitted.values, y_true = X.train_red$quality)
```
...
indicator(model = model.back_red, y_pred = model.back_red$fitted.values, y_true = X.train_red$quality)
cat("Correlation between predicted and actual values using TRAIN dataset\n")
cor(model.back_red$fitted.values, X.train_red$quality)

redPredict.back <- predict(model.back_red, newdata = X.test_red)
cat("\nPerformances using test dataset:\n")
indicator(model = model.back_red, y_pred = redPredict.back, y_true = X.test_red$quality)
cat("Correlation between predicted and actual values using TEST dataset\n")
cor(redPredict.back, X.test_red$quality)
```
